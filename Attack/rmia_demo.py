"""
End-to-end demo for RMIA using the project's minimal attack interface.

This demo mirrors the style of `shadow_based_demo.py` and `lira_demo.py`:
1. Load a real tabular dataset.
2. Save it to CSV if needed, then reload from CSV.
3. Train a target MLP classifier.
4. Build reference-model inputs for RMIA.
5. Prepare attacked samples and population samples.
6. Run RMIA and evaluate attack performance.

Run
---
python Attack/rmia_demo.py
"""
...
from Attack.rmia import AttackInput, RMIAAttack
...
class TabularMLP(nn.Module):
    """Small classifier used for target and reference models."""
...
def main() -> None:
    ...
    # Step 5: prepare attacked samples and population samples.
    rng = np.random.default_rng(seed + 1)
    nonmember_idx = rng.choice(len(x_test), size=min(len(x_train), len(x_test)) // 2, replace=False)
    remaining_test_idx = np.setdiff1d(np.arange(len(x_test)), nonmember_idx)
    population_idx = rng.choice(remaining_test_idx, size=min(128, len(remaining_test_idx)), replace=False)

    x_test_attack = x_test[nonmember_idx]
    y_test_attack = y_test[nonmember_idx]
    x_population = x_test[population_idx]
    y_population = y_test[population_idx]

    attack_samples = np.concatenate([x_train[: len(x_test_attack)], x_test_attack], axis=0)
    attack_labels = np.concatenate([y_train[: len(x_test_attack)], y_test_attack], axis=0)
    membership_labels = np.concatenate(
        [
            np.ones(len(x_test_attack), dtype=np.int64),
            np.zeros(len(x_test_attack), dtype=np.int64),
        ],
        axis=0,
    )

    # Global indices follow the reference-manager convention:
    # train indices in [0, len(x_train)), test indices offset by len(x_train).
    member_indices = np.arange(len(x_test_attack), dtype=np.int64)
    nonmember_indices = nonmember_idx.astype(np.int64) + len(x_train)
    sample_indices = np.concatenate([member_indices, nonmember_indices], axis=0)
    population_indices = population_idx.astype(np.int64) + len(x_train)
    ...
    attack_input = AttackInput(
        ...
        reference_data={
            ...
            "batch_size": 64,
            "population_samples": torch.tensor(x_population, dtype=torch.float32),
            "population_labels": torch.tensor(y_population, dtype=torch.long),
        },
        config={
            "batch_size": 64,
            "a": 0.0,
    ...
    print(f"Population samples:      {len(x_population)}")
    ...
    if attack_output.intermediate_outputs is not None:
        print(f"Reference Coverage:      {attack_output.intermediate_outputs['reference_coverage']:.4f}")
        print(
            f"Population Ref Coverage: "
            f"{attack_output.intermediate_outputs['population_reference_coverage']:.4f}"
        )

    print("\nFirst 10 RMIA scores:")
