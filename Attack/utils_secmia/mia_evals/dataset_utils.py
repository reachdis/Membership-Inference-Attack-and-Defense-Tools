...
class MIAMNIST(torchvision.datasets.MNIST):

    def __init__(self, idxs, **kwargs):
        super(MIAMNIST, self).__init__(**kwargs)
        self.idxs = idxs
...
def load_member_data(dataset_root, dataset_name, batch_size=128, member_split_root='./Attack/utils_secmia/mia_evals/member_splits/', shuffle=False, randaugment=False):
    if dataset_name.upper() == 'CIFAR10':
        ...
        member_idxs = splits['mia_train_idxs']
        nonmember_idxs = splits['mia_test_idxs']
        # load MIA Datasets
        transforms = torchvision.transforms.Compose([
        ...
        nonmember_set = MIASTL10(nonmember_idxs, root=os.path.join(dataset_root, 'stl10'), split='train',
                                 download=True, transform=transforms)
    elif dataset_name.upper() == 'STL10_U':
        ...
        nonmember_set = MIAImageFolder(nonmember_idxs, root=os.path.join(dataset_root, 'tiny-imagenet-200/train'),
                                       transform=transforms)
    elif dataset_name.upper() == 'NWPU':
        splits = np.load(os.path.join(member_split_root, 'NWPU_train_ratio0.5.npz'))
        member_idxs = splits['mia_train_idxs']
        nonmember_idxs = splits['mia_test_idxs']
        transforms = torchvision.transforms.Compose([
            torchvision.transforms.Resize(32),
        ...
        member_set = MIAImageFolder(member_idxs, root=nwpu_root, transform=transforms)
        nonmember_set = MIAImageFolder(nonmember_idxs, root=nwpu_root, transform=transforms)
    elif dataset_name.upper() == 'MNIST':
        splits = np.load(os.path.join(member_split_root, 'MNIST_train_ratio0.5.npz'))
        member_idxs = splits['mia_train_idxs']
        nonmember_idxs = splits['mia_test_idxs']
        transforms = torchvision.transforms.Compose([
            torchvision.transforms.Resize(32),
        ...
        member_set = MIAMNIST(member_idxs, root=os.path.join(dataset_root, 'mnist'), train=True,
                                 transform=transforms, download=True)
        nonmember_set = MIAMNIST(nonmember_idxs, root=os.path.join(dataset_root, 'mnist'), train=True,
                                 transform=transforms, download=True)
    else:
