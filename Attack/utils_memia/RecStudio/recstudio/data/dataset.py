...
class MFDataset(Dataset):
    ...
    def __init__(self, name: str='ml-100k', config:Union[Dict, str]=None):
        ...
        if cache_flag:
            ...
            #print_logger.info("Load dataset from cache.")
    ...
    def gen_idx2idx(self, oral_fiid2token2idx=None):
        ...
        if oral_fiid2token2idx is not None:
            ...
            #self.id2id = torch.LongTensor(self.id2id.detach().clone())
    ...
    def loader(self, batch_size, shuffle=True, num_workers=1, drop_last=False, ddp=False, active='no_active'):
        # if not ddp:
        ...
        if active == 'active_sampler':
            if hasattr(self, 'active_index'):
                sampler = ActiveDataSampler(self, batch_size, shuffle, drop_last, index=self.active_index)
            else:
                sampler = ActiveDataSampler(self, batch_size, shuffle, drop_last)
...
class ActiveDataSampler(Sampler):
    ...
    def __iter__(self):
        ...
        if self.index is not None:
            new_output = []
            for o in output:
                if o.item() in self.index:
                    new_output.append(o.item())
            output = torch.LongTensor(np.array(new_output))
        #-----
